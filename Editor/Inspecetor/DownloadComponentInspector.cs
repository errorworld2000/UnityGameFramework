...
namespace UnityGameFramework.Editor
{
    ...
    internal sealed class DownloadComponentInspector : GameFrameworkInspector
    {
                        ...
                        if (GUILayout.Button("Export CSV Data"))
                        {
                            ...
                            if (!string.IsNullOrEmpty(exportFileName))
                            {
                                try
                                {
                                    ...
                                    foreach (TaskInfo downloadInfo in downloadInfos)
                                    {
                                        data[index++] = string.Format("{0},{1},{2},{3}", downloadInfo.Description, downloadInfo.SerialId.ToString(), downloadInfo.Priority.ToString(), downloadInfo.Status.ToString());
                                    }

                                    File.WriteAllLines(exportFileName, data, Encoding.UTF8);
                                    Debug.Log(string.Format("Export download task CSV data to '{0}' success.", exportFileName));
                                }
                                catch (Exception exception)
                                {
                                    Debug.LogError(string.Format("Export download task CSV data to '{0}' failure, exception is '{1}'.", exportFileName, exception.ToString()));
                                }
                            }
                        }
                    }
        ...
        {
            EditorGUILayout.LabelField(downloadInfo.Description, string.Format("[SerialId]{0} [Priority]{1} [Status]{2}", downloadInfo.SerialId.ToString(), downloadInfo.Priority.ToString(), downloadInfo.Status.ToString()));
        }

...
namespace UnityGameFramework.Editor
{
    ...
    internal sealed class ResourceComponentInspector : GameFrameworkInspector
    {
                            ...
                            if (GUILayout.Button("Export CSV Data"))
                            {
                                ...
                                if (!string.IsNullOrEmpty(exportFileName))
                                {
                                    try
                                    {
                                        ...
                                        foreach (TaskInfo loadAssetInfo in loadAssetInfos)
                                        {
                                            data[index++] = string.Format("{0},{1},{2},{3}", loadAssetInfo.Description, loadAssetInfo.SerialId.ToString(), loadAssetInfo.Priority.ToString(), loadAssetInfo.Status.ToString());
                                        }

                                        File.WriteAllLines(exportFileName, data, Encoding.UTF8);
                                        Debug.Log(string.Format("Export load asset task CSV data to '{0}' success.", exportFileName));
                                    }
                                    catch (Exception exception)
                                    {
                                        Debug.LogError(string.Format("Export load asset task CSV data to '{0}' failure, exception is '{1}'.", exportFileName, exception.ToString()));
                                    }
                                }
                            }
                        }
        ...
        {
            EditorGUILayout.LabelField(loadAssetInfo.Description, string.Format("[SerialId]{0} [Priority]{1} [Status]{2}", loadAssetInfo.SerialId.ToString(), loadAssetInfo.Priority.ToString(), loadAssetInfo.Status.ToString()));
        }

...
namespace UnityGameFramework.Editor
{
    ...
    {
        private readonly Dictionary<string, List<ReferencePoolInfo>> m_ReferencePoolInfos = new(StringComparer.Ordinal);
                ...
                foreach (KeyValuePair<string, List<ReferencePoolInfo>> assemblyReferencePoolInfo in m_ReferencePoolInfos)
                {
                    ...
                    if (currentState)
                    {
                        EditorGUILayout.BeginVertical("box");
                        {
                            ...
                            {
                                string exportFileName = EditorUtility.SaveFilePanel("Export CSV Data", string.Empty, string.Format("Reference Pool Data - {0}.csv", assemblyReferencePoolInfo.Key), string.Empty);
        ...
        {
            EditorGUILayout.LabelField(m_ShowFullClassName ? referencePoolInfo.Type.FullName : referencePoolInfo.Type.Name, string.Format("{0}\t{1}\t{2}\t{3}\t{4}\t{5}", referencePoolInfo.UnusedReferenceCount.ToString(), referencePoolInfo.UsingReferenceCount.ToString(), referencePoolInfo.AcquireReferenceCount.ToString(), referencePoolInfo.ReleaseReferenceCount.ToString(), referencePoolInfo.AddReferenceCount.ToString(), referencePoolInfo.RemoveReferenceCount.ToString()));
        }

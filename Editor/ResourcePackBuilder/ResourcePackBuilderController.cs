    ...
    public sealed class ResourcePackBuilderController
    {
        ...
        public string SourcePath
        {
            get
            {
                ...
                return GameFramework.Utility.Path.GetRegularPath(new DirectoryInfo(string.Format("{0}/Full/", WorkingDirectory)).FullName);
            }
        }
        ...
        public string SourcePathForDisplay
        {
            get
            {
                ...
                return GameFramework.Utility.Path.GetRegularPath(new DirectoryInfo(string.Format("{0}/Full/*/{1}/", WorkingDirectory, Platform.ToString())).FullName);
            }
        }
        ...
        public string OutputPath
        {
            get
            {
                ...
                return GameFramework.Utility.Path.GetRegularPath(new DirectoryInfo(string.Format("{0}/ResourcePack/{1}/", WorkingDirectory, Platform.ToString())).FullName);
            }
        }

        public event Action<int> OnBuildResourcePacksStarted = null;

        public event Action<int, int> OnBuildResourcePacksCompleted = null;

        public event Action<int, int, string, string> OnBuildResourcePackSuccess = null;

        public event Action<int, int, string, string> OnBuildResourcePackFailure = null;
        ...
        public string[] GetVersionNames()
        {
            ...
            foreach (DirectoryInfo directoryInfo in sourceDirectoryInfo.GetDirectories())
            {
                ...
                DirectoryInfo platformDirectoryInfo = new DirectoryInfo(System.IO.Path.Combine(directoryInfo.FullName, platformName));
        ...
        public bool BuildResourcePack(string sourceVersion, string targetVersion)
        {
            ...
            string defaultResourcePackName = System.IO.Path.Combine(OutputPath, string.Format("{0}.{1}", DefaultResourcePackName, DefaultExtension));
            ...
            {
                DirectoryInfo sourceDirectoryInfo = new DirectoryInfo(System.IO.    Path.Combine(System.IO.Path.Combine(SourcePath, sourceVersion), Platform.ToString()));
                ...
                sourceVersionListBytes = Zip.Decompress(sourceVersionListBytes);
            ...
            DirectoryInfo targetDirectoryInfo = new DirectoryInfo(System.IO.Path.Combine(System.IO.Path.Combine(SourcePath, targetVersion), Platform.ToString()));
            ...
            targetVersionListBytes = Zip.Decompress(targetVersionListBytes);
            ...
            using (FileStream fileStream = new FileStream(defaultResourcePackName, FileMode.Open, FileAccess.ReadWrite))
            {
                ...
                {
                    string resourcePath = System.IO.Path.Combine(targetDirectoryPath, GetResourceFullName(resource.Name, resource.Variant, resource.HashCode));
                ...
                fileStream.Position = position;
                hashCode = Verifier.GetCrc32(fileStream);
            ...
            string targetResourcePackName = System.IO.Path.Combine(OutputPath, string.Format("{0}-{1}-{2}.{3:x8}.{4}", DefaultResourcePackName, sourceVersion ?? GetNoneVersion(targetVersion), targetVersion, hashCode, DefaultExtension));
        ...
        {
            return !string.IsNullOrEmpty(variant) ? string.Format("{0}.{1}.{3:x8}.{2}", name, variant, DefaultExtension, hashCode) : string.Format("{0}.{2:x8}.{1}", name, DefaultExtension, hashCode);
        }
    }
}

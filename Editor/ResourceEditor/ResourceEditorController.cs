...
namespace UnityGameFramework.Editor.ResourceTools
{
    public sealed class ResourceEditorController
    {
        ...
        {
            m_ConfigurationPath = Type.GetConfigurationPath<ResourceEditorConfigPathAttribute>() ?? GameFramework.Utility.Path.GetRegularPath(Path.Combine(Application.dataPath, "GameFramework/Configs/ResourceEditor.xml"));
        ...
        public event Action<SourceAsset[]> OnAssetAssigned = null;

        public event Action<SourceAsset[]> OnAssetUnassigned = null;
        ...
        private void RefreshSourceAssetSearchPaths()
        {
            ...
            if (m_SourceAssetSearchRelativePaths.Count > 0)
            {
                foreach (string sourceAssetSearchRelativePath in m_SourceAssetSearchRelativePaths)
                {
                    m_SourceAssetSearchPaths.Add(GameFramework.Utility.Path.GetRegularPath(Path.Combine(m_SourceAssetRootPath, sourceAssetSearchRelativePath)));
                }
            }
            else
            {

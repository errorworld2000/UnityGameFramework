    ...
    internal sealed partial class ResourceEditor : EditorWindow
    {
        ...
        private void OnGUI()
        {
            EditorGUILayout.BeginHorizontal(GUILayout.Width(position.width), GUILayout.Height(position.height));
            {
                ...
                {
                    GUILayout.Space(5f);
                    EditorGUILayout.LabelField(string.Format("Resource List ({0})", m_Controller.ResourceCount.ToString()), EditorStyles.boldLabel);
                ...
                {
                    GUILayout.Space(5f);
                    EditorGUILayout.LabelField(string.Format("Resource Content ({0})", m_CurrentResourceContentCount.ToString()), EditorStyles.boldLabel);
                    EditorGUILayout.BeginHorizontal("box", GUILayout.Height(position.height - 52f));
                    {
                        DrawResourceView();
                    }
        ...
        private void RefreshResourceTree()
        {
            ...
            foreach (Resource resource in resources)
            {
                ...
                string fullName = resource.Variant != null ? string.Format("{0}.{1}", splitedPath[splitedPath.Length - 1], resource.Variant) : splitedPath[splitedPath.Length - 1];

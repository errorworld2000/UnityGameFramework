...
namespace UnityGameFramework.Editor.ResourceTools
{
    ...
    internal sealed class ResourceBuilder : EditorWindow
    {
        ...
        {
            if (EditorUtility.DisplayCancelableProgressBar("Processing Binary", string.Format("Processing '{0}'...", binaryName), progress))
            {
                EditorUtility.ClearProgressBar();
                return true;
            }
        ...
        {
            EditorUtility.ClearProgressBar();
            Debug.Log(string.Format("Build resources for '{0}' complete.", platform.ToString()));
        }

        private void OnBuildResourceError(string errorMessage)
        {
            EditorUtility.ClearProgressBar();
            Debug.LogWarning(string.Format("Build resources error with error message '{0}'.", errorMessage));
        }
    }
}

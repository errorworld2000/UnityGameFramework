...
using GameFramework.Resource.DataStruct;
...
using Version = GameFramework.Base.Version.Version;
...
namespace UnityGameFramework.Editor.ResourceTools
{
    public sealed partial class ResourceBuilderController
    {
        ...
        {
            m_ConfigurationPath = Type.GetConfigurationPath<ResourceBuilderConfigPathAttribute>() ?? GameFramework.Utility.Path.GetRegularPath(System.IO.Path.Combine(Application.dataPath, "GameFramework/Configs/ResourceBuilder.xml"));

            m_ResourceCollection = new ResourceCollection();
            Zip.SetZipHelper(new DefaultZipHelper());
        ...
        public string WorkingPath
        {
            get
            {
                ...
                return GameFramework.Utility.Path.GetRegularPath(new DirectoryInfo(string.Format("{0}/Working/", OutputDirectory)).FullName);
            }
        }
        ...
        public string OutputPackagePath
        {
            get
            {
                ...
                return GameFramework.Utility.Path.GetRegularPath(new DirectoryInfo(string.Format("{0}/Package/{1}_{2}/", OutputDirectory, ApplicableGameVersion.Replace('.', '_'), InternalResourceVersion.ToString())).FullName);
            }
        }
        ...
        public string OutputFullPath
        {
            get
            {
                ...
                return GameFramework.Utility.Path.GetRegularPath(new DirectoryInfo(string.Format("{0}/Full/{1}_{2}/", OutputDirectory, ApplicableGameVersion.Replace('.', '_'), InternalResourceVersion.ToString())).FullName);
            }
        }
        ...
        public string OutputPackedPath
        {
            get
            {
                ...
                return GameFramework.Utility.Path.GetRegularPath(new DirectoryInfo(string.Format("{0}/Packed/{1}_{2}/", OutputDirectory, ApplicableGameVersion.Replace('.', '_'), InternalResourceVersion.ToString())).FullName);
            }
        }
        ...
        public string BuildReportPath
        {
            get
            {
                ...
                return GameFramework.Utility.Path.GetRegularPath(new DirectoryInfo(string.Format("{0}/BuildReport/{1}_{2}/", OutputDirectory, ApplicableGameVersion.Replace('.', '_'), InternalResourceVersion.ToString())).FullName);
            }
        }
        ...
        public bool Save()
        {
            try
            {
                ...
                string configurationDirectoryName = System.IO.Path.GetDirectoryName(m_ConfigurationPath);
        ...
        public bool RefreshBuildEventHandler()
        {
            ...
            {
                System.Type buildEventHandlerType = Assembly.GetType(BuildEventHandlerTypeName);
        ...
        private bool BuildResources(Platform platform, AssetBundleBuild[] assetBundleBuildDatas, BuildAssetBundleOptions buildAssetBundleOptions, ResourceData[] assetBundleResourceDatas, ResourceData[] binaryResourceDatas)
        {
            ...
            if (Directory.Exists(workingPath))
            {
                ...
                GameFramework.Utility.Path.RemoveEmptyDirectory(workingPath);
            }
        ...
        private bool ProcessAssetBundle(Platform platform, string workingPath, string outputPackagePath, string outputFullPath, string outputPackedPath, bool zip, string name, string variant, string fileSystem)
        {
            ...
            string workingName = GameFramework.Utility.Path.GetRegularPath(System.IO.Path.Combine(workingPath, fullName.ToLower()));

            byte[] bytes = File.ReadAllBytes(workingName);
            int length = bytes.Length;
            int hashCode = Verifier.GetCrc32(bytes);
            int zipLength = length;
            int zipHashCode = hashCode;

            byte[] hashBytes = Converter.GetBytes(hashCode);
            if (resourceData.LoadType == LoadType.LoadFromMemoryAndQuickDecrypt)
            {
                bytes = Encryption.GetQuickXorBytes(bytes, hashBytes);
            }
            else if (resourceData.LoadType == LoadType.LoadFromMemoryAndDecrypt)
            {
                bytes = Encryption.GetXorBytes(bytes, hashBytes);
            }
        ...
        private bool ProcessBinary(Platform platform, string workingPath, string outputPackagePath, string outputFullPath, string outputPackedPath, bool zip, string name, string variant, string fileSystem)
        {
            ...
            string assetPath = GameFramework.Utility.Path.GetRegularPath(Application.dataPath.Substring(0, Application.dataPath.Length - AssetsStringLength) + assetName);
        ...
        private void ProcessPackageVersionList(string outputPackagePath, Platform platform)
        {
            ...
            string packageVersionListPath = GameFramework.Utility.Path.GetRegularPath(System.IO.Path.Combine(outputPackagePath, RemoteVersionListFileName));
        ...
        private VersionListData ProcessUpdatableVersionList(string outputFullPath, Platform platform)
        {
            ...
            string updatableVersionListPath = GameFramework.Utility.Path.GetRegularPath(System.IO.Path.Combine(outputFullPath, RemoteVersionListFileName));
            ...
            int zipHashCode = Verifier.GetCrc32(bytes);
            int dotPosition = RemoteVersionListFileName.LastIndexOf('.');
            string versionListFullNameWithCrc32 = string.Format("{0}.{2:x8}.{1}", RemoteVersionListFileName.Substring(0, dotPosition), RemoteVersionListFileName.Substring(dotPosition + 1), hashCode);
            string updatableVersionListPathWithCrc32 = GameFramework.Utility.Path.GetRegularPath(System.IO.Path.Combine(outputFullPath, versionListFullNameWithCrc32));
        ...
        private void ProcessReadOnlyVersionList(string outputPackedPath, Platform platform)
        {
            ...
            string readOnlyVersionListPath = GameFramework.Utility.Path.GetRegularPath(System.IO.Path.Combine(outputPackedPath, LocalVersionListFileName));
        ...
        private void CreateFileSystems(IEnumerable<ResourceData> resourceDatas, string outputPath, Dictionary<string, IFileSystem> outputFileSystem)
        {
            ...
            foreach (string fileSystemName in fileSystemNames)
            {
                ...
                string fullPath = GameFramework.Utility.Path.GetRegularPath(System.IO.Path.Combine(outputPath, string.Format("{0}.{1}", fileSystemName, DefaultExtension)));
        ...
        {
            string fullNameWithExtension = string.Format("{0}.{1}", GetResourceFullName(name, variant), GetExtension(resourceData));

            if (OutputPackageSelected)
            {
                if (string.IsNullOrEmpty(fileSystem))
                {
                    string packagePath = GameFramework.Utility.Path.GetRegularPath(System.IO.Path.Combine(outputPackagePath, fullNameWithExtension));
                    string packageDirectoryName = System.IO.Path.GetDirectoryName(packagePath);
            ...
            if (OutputPackedSelected && resourceData.Packed)
            {
                if (string.IsNullOrEmpty(fileSystem))
                {
                    string packedPath = GameFramework.Utility.Path.GetRegularPath(System.IO.Path.Combine(outputPackedPath, fullNameWithExtension));
                    string packedDirectoryName = System.IO.Path.GetDirectoryName(packedPath);
            ...
            if (OutputFullSelected)
            {
                string fullNameWithCrc32AndExtension = variant != null ? string.Format("{0}.{1}.{2:x8}.{3}", name, variant, hashCode, DefaultExtension) : string.Format("{0}.{1:x8}.{2}", name, hashCode, DefaultExtension);
                string fullPath = GameFramework.Utility.Path.GetRegularPath(System.IO.Path.Combine(outputFullPath, fullNameWithCrc32AndExtension));
                string fullDirectoryName = System.IO.Path.GetDirectoryName(fullPath);
                ...
                if (zip)
                {
                    byte[] zipBytes = Zip.Compress(bytes);
                    zipLength = zipBytes.Length;
                    zipHashCode = Verifier.GetCrc32(zipBytes);
        ...
        private bool PrepareBuildData(out AssetBundleBuild[] assetBundleBuildDatas, out ResourceData[] assetBundleResourceDatas, out ResourceData[] binaryResourceDatas)
        {
            ...
            foreach (Asset asset in assets)
            {
                ...
                int assetHashCode = Verifier.GetCrc32(assetBytes);

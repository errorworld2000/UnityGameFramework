        ...
        private void DrawAssetDependencyViewer()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                EditorGUILayout.BeginVertical(GUILayout.Width(position.width * 0.4f));
                {
                    ...
                    {
                        title = string.Format("Assets In Resources ({0})", m_AssetCount.ToString());
                    }
                    else
                    {
                        title = string.Format("Assets In Resources ({0}/{1})", m_CachedAssetNames.Length.ToString(), m_AssetCount.ToString());
                    }
                ...
                {
                    GUILayout.Space(5f);
                    EditorGUILayout.LabelField(string.Format("Dependency Resources ({0})", m_SelectedDependencyData.DependencyResourceCount.ToString()), EditorStyles.boldLabel);
                        ...
                        m_HostAssetsScroll = EditorGUILayout.BeginScrollView(m_HostAssetsScroll);
                        {
                            foreach (Asset hostAsset in m_SelectedHostAssets)
                            {
                                EditorGUILayout.BeginHorizontal();
                                {
                                    ...
                                    GUILayout.Label(string.Format("{0} [{1}]", hostAsset.Name, hostAsset.Resource.FullName));
                                }
                                EditorGUILayout.EndHorizontal();
                            }
                        }

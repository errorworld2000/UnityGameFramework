...
using GameFramework.Resource.Enums;
...
namespace UnityGameFramework.Runtime
{
    ...
    [AddComponentMenu("Game Framework/Editor Resource")]
...
#pragma warning restore 0067, 0414
            ...
            if (m_LoadSceneInfos.Count > 0)
            {
                ...
                while (current != null)
                {
                    ...
                    if (loadSceneInfo.AsyncOperation.isDone)
                    {
                        ...
                        {
                            loadSceneInfo.LoadSceneCallbacks.LoadSceneSuccessCallback?.Invoke(loadSceneInfo.SceneAssetName, (float)(DateTime.UtcNow - loadSceneInfo.StartTime).TotalSeconds, loadSceneInfo.UserData);
                        }
                        else
                        {
        ...
        public void LoadAsset(string assetName, Type assetType, int priority, LoadAssetCallbacks loadAssetCallbacks, object userData)
        {
            ...
            if (!assetName.StartsWith("Assets/", StringComparison.Ordinal))
            {
                if (loadAssetCallbacks.LoadAssetFailureCallback != null)
                {
                    loadAssetCallbacks.LoadAssetFailureCallback(assetName, LoadResourceStatus.NotExist, string.Format("Asset name '{0}' is invalid.", assetName), userData);
                }
            ...
            if (!HasFile(assetName))
            {
                if (loadAssetCallbacks.LoadAssetFailureCallback != null)
                {
                    loadAssetCallbacks.LoadAssetFailureCallback(assetName, LoadResourceStatus.NotExist, string.Format("Asset '{0}' is not exist.", assetName), userData);
                }

                return;
            }

            m_LoadAssetInfos.AddLast(new LoadAssetInfo(assetName, assetType, priority, DateTime.UtcNow, m_MinLoadAssetRandomDelaySeconds + (float)GameFramework.Utility.Random.GetRandomDouble() * (m_MaxLoadAssetRandomDelaySeconds - m_MinLoadAssetRandomDelaySeconds), loadAssetCallbacks, userData));
        }
        ...
        public void LoadScene(string sceneAssetName, int priority, LoadSceneCallbacks loadSceneCallbacks, object userData)
        {
            ...
            if (!sceneAssetName.StartsWith("Assets/", StringComparison.Ordinal) || !sceneAssetName.EndsWith(".unity", StringComparison.Ordinal))
            {
                if (loadSceneCallbacks.LoadSceneFailureCallback != null)
                {
                    loadSceneCallbacks.LoadSceneFailureCallback(sceneAssetName, LoadResourceStatus.NotExist, string.Format("Scene asset name '{0}' is invalid.", sceneAssetName), userData);
                }
            ...
            if (!HasFile(sceneAssetName))
            {
                if (loadSceneCallbacks.LoadSceneFailureCallback != null)
                {
                    loadSceneCallbacks.LoadSceneFailureCallback(sceneAssetName, LoadResourceStatus.NotExist, string.Format("Scene '{0}' is not exist.", sceneAssetName), userData);
                }
        ...
        public void LoadBinary(string binaryAssetName, LoadBinaryCallbacks loadBinaryCallbacks, object userData)
        {
            ...
            if (!binaryAssetName.StartsWith("Assets/", StringComparison.Ordinal))
            {
                if (loadBinaryCallbacks.LoadBinaryFailureCallback != null)
                {
                    loadBinaryCallbacks.LoadBinaryFailureCallback(binaryAssetName, LoadResourceStatus.NotExist, string.Format("Binary asset name '{0}' is invalid.", binaryAssetName), userData);
                }
            ...
            if (binaryPath == null)
            {
                if (loadBinaryCallbacks.LoadBinaryFailureCallback != null)
                {
                    loadBinaryCallbacks.LoadBinaryFailureCallback(binaryAssetName, LoadResourceStatus.NotExist, string.Format("Binary asset '{0}' is not exist.", binaryAssetName), userData);
                }
        ...
        private bool HasFile(string assetName)
        {
            ...
            string fileFullName = GameFramework.Utility.Path.GetRegularPath(fileNames[0]);
        ...
        private UnityEngine.Object GetCachedAsset(string assetName)
        {
            ...
            if (m_CachedAssets.TryGetValue(assetName, out UnityEngine.Object asset))
            {
                return asset;
            }
        ...
        private readonly struct LoadAssetInfo
        {
        ...
        [StructLayout(LayoutKind.Auto)]
        private readonly struct LoadSceneInfo
        {
        ...
        [StructLayout(LayoutKind.Auto)]
        private readonly struct UnloadSceneInfo
        {

...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed class AndroidFileSystemStream : FileSystemStream
    {
        ...
        public AndroidFileSystemStream(string fullPath, FileSystemAccess access, bool createNew)
        {
            ...
            {
                throw new GameFrameworkException(string.Format("'{0}' is not supported in AndroidFileSystemStream.", access.ToString()));
            }
            ...
            {
                throw new GameFrameworkException(string.Format("Open file '{0}' from Android asset manager failure.", fullPath));
            }
        ...
        private int InternalRead(int length, out byte[] result)
        {
            // 使用推荐的NewSByteArray创建数组
            IntPtr resultPtr = AndroidJNI.NewSByteArray(length);
            if (resultPtr == IntPtr.Zero)
            {
                throw new System.Exception("Failed to allocate byte array.");
            }
            ...
            // 转换sbyte[]到byte[]
            sbyte[] sbyteResult = AndroidJNI.FromSByteArray(resultPtr);
            result = new byte[sbyteResult.Length];
            Buffer.BlockCopy(sbyteResult, 0, result, 0, sbyteResult.Length); // 最高效的转换方式

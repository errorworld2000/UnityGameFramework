...
public static class BinaryExtension
{
    ...
    public static string ReadEncryptedString(this BinaryReader binaryReader, byte[] encryptBytes)
    {
        ...
        GameFramework.Utility.Encryption.GetSelfXorBytes(s_CachedBytes, 0, length, encryptBytes);
        string value = GameFramework.Utility.Converter.GetString(s_CachedBytes, 0, length);
    ...
    public static void WriteEncryptedString(this BinaryWriter binaryWriter, string value, byte[] encryptBytes)
    {
        ...
        int length = GameFramework.Utility.Converter.GetBytes(value, s_CachedBytes);
        if (length > byte.MaxValue)
        {
            throw new GameFrameworkException(string.Format("String '{0}' is too long.", value));
        }

        GameFramework.Utility.Encryption.GetSelfXorBytes(s_CachedBytes, encryptBytes);
